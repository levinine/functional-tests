package com.levi9.functionaltests.rest.proxy.petservice;

import static com.levi9.functionaltests.rest.data.pet.PetStatus.PENDING;
...
public class PetServiceProxy {
	...
	public void addPetToStore(final String petName) {
		...
		final Response response = petStoreRestClient.post(body, null, REST_PET);
	...
	public void updatePetStatus(final PetEntity pet, final PetStatus petStatus) {
		...
		final Response response = petStoreRestClient.put(body, null, REST_PET);
		if (response.statusCode() != HttpStatus.SC_OK) {
	...
	public PetDSO getPet(final PetEntity pet) {
		...
		final Response response = petStoreRestClient.get(null, REST_PET + pet.getId());
		if (response.statusCode() != HttpStatus.SC_OK) {
	...
	public boolean removePet(final PetEntity pet) {
		final Response response = petStoreRestClient.delete(null, REST_PET + pet.getId());
	...
	public boolean uploadPetImage(final PetEntity pet, final String fileName) {
		try {
			...
			final Response response =
				petStoreRestClient.uploadFile(imageFile, null, REST_PET + pet.getId() + "/uploadImage");
			return response.statusCode() == HttpStatus.SC_OK;
